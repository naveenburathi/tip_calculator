...
const LeftComponent = ({splitterObj,setSplitterObj}) => {
    ...
    <Grid container direction={'column'} sx={{'padding-right':'10px'}} spacing={2.5}>
        ...
        <Grid container direction={'row'} spacing={3} alignItems={'stretch'}>
            ...
            <Grid item xs={4}><TextField value={splitterObj.customTip} onChange={handleTipChange} id="outlined-basic" placeholder='Custom' variant="outlined" /></Grid>
        </Grid>
        </Grid>
        <Grid item>
        ...
        {splitterObj.errorNoOfPeople&&(<Grid item xs={6}><FormHelperText sx={{'textAlign':'right','margin-right':'0','font-family':'Space Mono'}} error>Can't be zero</FormHelperText></Grid>)}
        </Grid>
        <OutlinedInput
            className='text-input'
            id="bill"
          ...
          error={splitterObj.errorNoOfPeople}
            sx={{
                'fontFamily':'Space Mono',
                'font-size': '1.3rem',
                'font-weight': 'bold',
